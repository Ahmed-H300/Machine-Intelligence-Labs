...
class SnakeEnv(Environment[SnakeObservation, Direction]):
    ...
    def reset(self, seed: Optional[int] = None) -> Point:
        ...
        # My-Comment: because generate_random_apple uses the snake variable!
        # My-Comment: Initialize the snake at the center of the grid, moving left
        self.snake = [Point(self.width // 2, self.height // 2)]
        self.direction = Direction.LEFT
        # My-Comment: Generate a random apple that is not on the snake's body
    ...
    def actions(self) -> List[Direction]:
        ...
        # My-Comment: The snake can not move in the opposite direction of its current direction
        # My-Comment: The snake can not move in the same direction
        # My-Comment: define the opposite move for each move
        opposite_direction = {
        ...
        # My-Comment: remove opposite move and the move I am in
        return [d for d in Direction if d != opposite_direction[self.direction] and d != self.direction]
        # NotImplemented()
    ...
    def step(self, action: Direction) -> \
        ...
        reward = 0
        # My-Comment: Move the snake in the direction of the action
        # My-Comment: If the action is None, continue moving in the current direction
        action_to_make = self.direction if action == Direction.NONE else action
        # My-Comment: define the directions value
        new_head = self.snake[0] + action_to_make.to_vector()
        # My-Comment: Check if it is a wall wrap to the other side
        if new_head.x < 0:
        ...
        # My-Comment: Check if the snake has bitten itself
        if new_head in self.snake:
            # My-Comment: Decrease the reward and end the game
            reward -= 100
            done = True
        else:
            # My-Comment: Add the new head to the snake
            self.snake.insert(0, new_head)
        
        # My-Comment: Check if the snake has eaten the apple
        if new_head == self.apple:
            # My-Comment: Increase the reward and generate a new apple
            reward += 1
            # My-Comment: Check if the snake has won the game
            if len(self.snake) == self.width * self.height:
                # My-Comment: Increase the reward and end the game
                reward += 100
                done = True
            else:
                self.apple = self.generate_random_apple()
        else:
            # My-Comment: If the snake did not eat the apple, remove the tail
            self.snake.pop()

        # My-Comment: Update the direction
        self.direction = action_to_make
